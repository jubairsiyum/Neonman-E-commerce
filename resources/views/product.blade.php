    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
        ...
        <div>
            ...
            <div class="flex flex-col sm:flex-row gap-3 mb-6 sm:mb-8">
                @if($product->in_stock)
                <button onclick="addToCartFromProductPage()" class="flex-1 px-4 sm:px-6 py-3 sm:py-4 bg-gray-900 hover:bg-gray-800 dark:bg-white dark:hover:bg-gray-200 dark:text-gray-900 text-white font-bold text-sm sm:text-base rounded transition-colors flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                    </svg>
                    Add to Cart
                </button>
                <button onclick="buyNowFromProductPage()" class="flex-1 px-4 sm:px-6 py-3 sm:py-4 bg-primary-900 hover:bg-primary-950 text-white font-bold text-sm sm:text-base rounded transition-colors flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @else
                <button disabled class="flex-1 px-4 sm:px-6 py-3 sm:py-4 bg-gray-400 text-white font-bold text-sm sm:text-base rounded cursor-not-allowed">
                    OUT OF STOCK
                </button>
                @endif
...
function addToCartFromProductPage() {
    ...
    @if($product->sizes && is_array($product->sizes) && count($product->sizes) > 0 && collect($product->sizes)->flatten()->filter(fn($s) => is_string($s))->isNotEmpty())
    if (!selectedSize) {
    ...
    @if($product->colors && is_array($product->colors) && count($product->colors) > 0 && collect($product->colors)->flatten()->filter(fn($c) => is_string($c))->isNotEmpty())
    if (!selectedColor) {
...
function buyNowFromProductPage() {
    ...
    @if($product->sizes && is_array($product->sizes) && count($product->sizes) > 0 && collect($product->sizes)->flatten()->filter(fn($s) => is_string($s))->isNotEmpty())
    if (!selectedSize) {
        showToast('Please select a size before ordering.', 'error');
        return;
    }
    @endif
    ...
    @if($product->colors && is_array($product->colors) && count($product->colors) > 0 && collect($product->colors)->flatten()->filter(fn($c) => is_string($c))->isNotEmpty())
    if (!selectedColor) {
        showToast('Please select a color before ordering.', 'error');
        return;
    }
    @endif

    addToCartAndCheckout({{ $product->id }}, quantity, selectedSize, selectedColor);
}
</script>
