...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            @foreach($order->items as $item)
                <div class="px-6 py-5 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Quantity: {{ $item->quantity }}</p>
                    </div>
                    <p class="font-semibold text-gray-900 dark:text-white shrink-0">Tk {{ number_format($item->total, 2) }}</p>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="space-y-5">
        ...
        <section class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
            ...
            <p class="text-sm text-gray-700 dark:text-gray-300 mt-1">{{ $order->shipping_district }}, {{ $order->shipping_division }} {{ $order->shipping_postal_code }}</p>
