...
@section('content')
<div class="mb-8">
    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Customer Dashboard</h1>
...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5 mb-8">
    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        ...
        <p class="mt-1 text-3xl font-bold text-gray-900 dark:text-white">{{ $stats['total_orders'] }}</p>
    </div>
    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        <p class="text-sm text-gray-500 dark:text-gray-400">Active Orders</p>
        <p class="mt-1 text-3xl font-bold text-gray-900 dark:text-white">{{ $stats['pending_orders'] }}</p>
    </div>
    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        <p class="text-sm text-gray-500 dark:text-gray-400">Wishlist Items</p>
        <p class="mt-1 text-3xl font-bold text-gray-900 dark:text-white">{{ $stats['wishlist_count'] }}</p>
    </div>
    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        <p class="text-sm text-gray-500 dark:text-gray-400">Total Spent</p>
        <p class="mt-1 text-3xl font-bold text-gray-900 dark:text-white">Tk {{ number_format($stats['total_spent'], 2) }}</p>
    </div>
</div>

<div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
    <div class="px-5 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Recent Orders</h2>
        <a href="{{ route('my-orders') }}" class="text-sm font-semibold text-primary-600 dark:text-primary-400 hover:underline">View all</a>
    </div>

    @if($recentOrders->isEmpty())
        <div class="p-8 text-center">
            <p class="text-gray-600 dark:text-gray-400">No orders yet.</p>
            <a href="{{ route('shop') }}" class="inline-flex mt-4 px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium">Start shopping</a>
        </div>
    @else
                    ...
                    @foreach($recentOrders as $order)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('order.detail', $order) }}" class="text-primary-600 dark:text-primary-400 hover:underline">Details</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
